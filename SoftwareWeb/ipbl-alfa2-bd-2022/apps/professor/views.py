    ...
    if request.POST:
        ...
        if len(professores):
            ...
            if True:
                ...
                response.set_cookie(
                    "identificador_professor", professor["identificador"]
                )
...
def coleta(request):
    ...
    professor = scripts_mongodb.db["professores"].find_one(
        {"identificador": identificador}
    )

    frases_tipo_1 = scripts_mongodb.get_data_find(
        collection_name="frases", filter={"tipo": 1}
    )
    frases_tipo_2 = scripts_mongodb.get_data_find(
        collection_name="frases", filter={"tipo": 2}
    )
    frases_tipo_3 = scripts_mongodb.get_data_find(
        collection_name="frases", filter={"tipo": 3}
    )
...
def view_audio_metrics(request):
    ...
    avaliacao = scripts_mongodb.get_object_by_id(
        collection_name="avaliacoes", _id=aluno["avaliacao"]
    )
...
def banco_frases(request):
    ...
    context = {"segment": "banco_frases", "frases": frases}
...
def informacoes(request):
    ...
    professor = scripts_mongodb.get_data_find(
        collection_name="professores", filter={"identificador": identificador}
    )[0]
...
def turmas(request):
    ...
    turmas = scripts_mongodb.get_data_find(
        collection_name="turmas", filter={"professor": professor}
    )
...
def alunos(request):
    ...
    if request.POST:
        ...
        context = {"segment": "alunos", "alunos": alunos}
