...
def main(*args, **kwargs):
    ...
    delete_elements_before = (
        False
        if "delete_elements_before" not in kwargs
        else kwargs["delete_elements_before"]
    )
    crypto = CryptoHelper()
    ...
    for collection in collections_list:
        if scripts_mongodb.verify_collection_in_db(collection_name=collection) == False:
            scripts_mongodb.create_collection(collection_name=collection)
    ...
    if scripts_mongodb.number_elements_collection(collection_name="frases") == 0:
        ...
        for i in range(number_of_profs):
            json_frase = {"frase": fake.catch_phrase(), "tipo": randint(1, 3)}
    ...
    if scripts_mongodb.number_elements_collection(collection_name="professores") == 0:
        ...
        for i in range(number_of_profs):
            ...
            collection_avaliacoes.update_one(
                {"_id": avaliacoes[i]["_id"]}, {"$set": {"professor": id}}
            )
            collection_avaliacoes.update_one(
                {"_id": avaliacoes[i + number_of_profs]["_id"]},
                {"$set": {"professor": id}},
            )
    ...
    if scripts_mongodb.number_elements_collection(collection_name="alunos") == 0:
        ...
        for i in range(2 * number_of_profs):
            ...
            # updating avaliacoes
            collection_avaliacoes.update_one(
                {"_id": avaliacoes[i]["_id"]}, {"$set": {"aluno": id}}
            )
    ...
    if scripts_mongodb.number_elements_collection(collection_name="turmas") == 0:
        ...
        for i in range(number_of_profs):
            ...
            json_turmas = {
                ...
                "alunos": [alunos[i]["_id"], alunos[i + number_of_profs]["_id"]],
                "professor": profs[i]["_id"],
            }
            obj_turmas.append(InsertOne(json_turmas))
            # adding turma to alunos
            collection_alunos.update_one(
                {"_id": alunos[i]["_id"]}, {"$set": {"turma": id}}
            )
            collection_alunos.update_one(
                {"_id": alunos[i + number_of_profs]["_id"]}, {"$set": {"turma": id}}
            )

            # adding turma to profs
            collection_professores.update_one(
                {"_id": profs[i]["_id"]}, {"$push": {"turma": id}}
            )
    ...
    if scripts_mongodb.number_elements_collection(collection_name="escolas") == 0:
        ...
        for i in range(number_of_profs - 1):
            ...
            json_escola = {
                ...
                "turmas": [turmas[i]["_id"], turmas[i + 1]["_id"]],
            }
    ...
    if scripts_mongodb.number_elements_collection(collection_name="gestores") == 0:
        ...
        for i in range(number_of_profs - 2):
            json_gestor = {
                ...
                "identificador": fake.cpf(),
                "hash_senha": crypto.encrypt_message("1234"),
                "escolas": [escolas[i]["_id"], escolas[i + 1]["_id"]],
            }
    ...
    if delete_elements_before:
        scripts_mongodb.delete_elements_from_collection(
            collection_name="gestores_admin"
        )

    if (
        scripts_mongodb.number_elements_collection(collection_name="gestores_admin")
        == 0
    ):
        obj_gestores_admin = []
        ...
        for i in range(number_of_profs - 3):
            json_gestor_admin = {
                ...
                "nome": fake.first_name(),
                "sobrenome": fake.last_name(),
                "gestor_escola": [gestores[i]["_id"], gestores[i + 1]["_id"]],
                "hash_senha": crypto.encrypt_message("1234"),
            }
